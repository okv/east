...
const describeTitle = (
	'migrator configure register plugins with absolute path'
);
...
describe(describeTitle, () => {
	...
	it('should be done without error', () => {
		return migratorMock.configure({plugins: ['/tmp/somePlugin']});
	});

	it('should try only absolute path', () => {
		expect(loadModulePaths).length(1);
		expect(loadModulePaths[0]).eql('/tmp/somePlugin');
	});
