...
MigrationManager.prototype.getMigrationPath = function getMigrationPath(name) {
	return Promise.resolve(
		this.migrator.getMigrationPathByName(name)
	);
};
...
MigrationManager.prototype.migrate = function migrate(params) {
	...
	return Promise.resolve()
		...
		.then((result) => {
			if (result.separated) {
				if (result.separated.executedNames.length) {
					this.emit('onSkipMigrations', {
						migrationNames: result.separated.executedNames
					});
				}
...
MigrationManager.prototype.rollback = function rollback(params) {
	...
	return Promise.resolve()
		...
		.then((result) => {
			if (result.separated) {
				if (result.separated.newNames.length) {
					this.emit('onSkipMigrations', {
						migrationNames: result.separated.newNames
					});
				}
