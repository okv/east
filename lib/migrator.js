...
Migrator.prototype._createAdapter = function(adapter, params) {
	// try migrator relative path first.
	var Adapter = this._tryLoadAdapter(adapter);
	if (Adapter instanceof Error) {
		// If not present then try CWD relative path.
		Adapter = this._tryLoadAdapter(path.join(process.cwd(), adapter));
		if (Adapter instanceof Error) {
			Adapter.message =
				'Error loading both `' + adapter + '` and `' + path.join(process.cwd(), adapter) + '`:\n' + Adapter.message;
			throw Adapter;
		}
	}
...
Migrator.prototype.validateMigration = function(migration, callback) {
	var msg = null;
	if (!utils.isObject(migration)) msg = 'migration is not an object';
