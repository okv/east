...
Migrator.prototype.create = function(baseName, callback) {
	...
	Steppy(
		...
		function(err, templateText) {
			var migrationPath = self.getMigrationPathByName(name);

			fs.writeFile(migrationPath, templateText, this.slot());
		},
		function(err) {
			this.pass(name);
		},
...
Migrator.prototype.remove = function(name, callback) {
	...
	Steppy(
		function() {
			...
			fs.unlink(migrationPath, this.slot());
		},
		callback
	);
};
...
Migrator.prototype.getMigrationPathByName = function(name) {
	return pathUtils.resolve(pathUtils.join(this.params.dir, name + '.js'));
};

Migrator.prototype.getMigrationNameByPath = function(mpath) {
	return pathUtils.basename(mpath, '.js');
};
...
Migrator.prototype.validateMigration = function(migration, callback) {
	...
	} else if (!utils.isFunction(migration.migrate)) {
		msg = '`migrate` is not a function';
	} else if (migration.rollback && !utils.isFunction(migration.rollback)) {
