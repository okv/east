...
Migrator.prototype.normalizeNames = function(names, callback) {
	this.getAllMigrationNames(function(err, allNames) {
		...
		var normalizedNames = names.map(function(name) {
			...
			var number = getNumberFromName(name);
			if (number && number in numbersHash) {
				if (numbersHash[name].length == 1) {
					return numbersHash[number][0];
				} else {
					callback(new Error(
						'Specified migration name `' + name + '` ambiguous: ' +
						numbersHash[number].join(',') + ' please choose one'
					));
				}
			}
			var baseName = getBaseNameFromName(name);
			if (baseName && baseName in baseNamesHash) {
...
function getNumberFromName(name) {
	var parts = /[0-9]+/.exec(name);
	return parts ? parts[0] : null;
}

function getBaseNameFromName(name) {
	var parts = /_.*/.exec(name);
	return parts ? parts[0].replace('_', '') : null;
}
