...
function Migrator(params) {
	...
	self.params = self._loadConfig(
		params.config || path.join(cwd, 'eastconf.json')
	);
	var defaults = {
		// migrations dir
...
Migrator.prototype.create = function(basename) {
	var name = Date.now() + '_' + basename;
	fs
		.createReadStream(this.params.template)
		.pipe(fs.createWriteStream(this.getMigrationPathByName(name)));
	return name;
}
