...
Migrator.prototype._promisifyAdapter = function (adapter) {
	if (adapter.connect) {
...
Migrator.prototype._createAdapter = function (adapter, params) {
	...
	let paths = [adapter, pathUtils.join(process.cwd(), adapter)],
		Adapter,
		errors = [];
...
Migrator.prototype._getNextNumber = function () {
	return Promise.resolve()
		...
		.then((allNames) => {
			...
			let num = this._getNumber(lastNames);
			num++;
...
Migrator.prototype.getNewMigrationNames = function () {
	return Promise.resolve()
		...
		.then((result) => {
			...
			result.executedNames.forEach((name) => {
	...
	return Promise.resolve()
		...
		.then((result) => {
			if (result.executedNames) {
				const isExecuted = (
					result.executedNames.indexOf(migration.name) !== -1
				);
